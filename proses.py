...
def menu(my_name,my_id):
	...
	cetak(nel('\twelcoming Scrip fb[yellow]%s[white] BAGI GUA 20K'%(my_name)))
	print(f'Your Idz : '+str(my_id))
	print(f'Your Ip  : {ip}')
	print('')
	print('1. XRACK Konnichiwa ')
	print('2. XRACK Irasshaimase ')
	print('3. XRACK Arigato')
	print('4. XRACK Moshi Moshi')
	print('5. XRACK Irasshaimase')
	print('0. 𝙺𝙴𝙻𝚄𝙰𝚁 (𝐇𝐚𝐩𝐮𝐬 𝐂𝐨𝐨𝐤𝐢𝐞)      ')
	_____Riski__xd_____ = input('\n|──> 𝐒𝐞𝐥𝐞𝐜𝐭 : ')
	if _____Riski__xd_____ in ['1']:
		dump_massal()
	elif _____alvino__adijaya_____ in ['2']:
		dump_follower()
	elif _____riski__xd_____ in ['3']:
		grup()
	elif _____riski__xd_____ in ['4']:
		crack_file()
	elif _____riski__xd_____ in ['5']:
		result()
	elif _____aruski__xd_____ in ['0']:
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('krek berapa target lu ? : '))
	except ValueError:
	...
	for met in range(jum):
		yz+=1
		kl = input('|masukin kontol ??𝐚𝐧𝐠 𝐊𝐞 : '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
	...
	try:
		print('')
		print(f'|𝐓𝐨𝐭𝐚𝐥 ??cie dapat idz bagus : {h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
		...
		print(f'>>{k} id busuk kek nya blom mandi lu {x}')
		time.sleep(3)
		back()
#-------------------[ CRACK-PENGIKUT ]----------------#
def dump_pengikut():
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{H}[RISKIXD] {P}{b}{loop}{P}/{u}{len(id)}{P}/{P}{H}{ok}{P}/{P}{k}{cp}{x}/{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
