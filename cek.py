...
###----------[ BANNER MENUH ]----------###
def banner():
	print('''SALAM DARI FIRMAN 
\x1b[1;91m____ ____ _  _ ___  _  _ _    ____ _  _    
|__/ |___ |\/| |__] |  | |    |__| |\ |    
\33[m|  \ |___ |  | |__] |__| |___ |  | | \|    
SC INI HANYA CEK CP FITUR CRACK GA ADA''')
...
def menu(id):
	...
	banner()
	print(f'{xxx}─────────────────────────────')
#	os.system("xdg-open https://chat.whatsapp.com/DJ2RWBRMTQDKJkBd5UfRai")
	print(f'{xxx}>> 1. cek mbasic.facebook')
	print(f'{xxx}>> 2. cek free.facebook')
	helpbas = input(f'{xxx}>> : ')
	if helpbas in ['1']:
		file_cp()
	elif helpbas in ['2']:
...
def file_cp():
	dirs = os.listdir('CP')
	print ("%s%s%s [%s\033[0mpilih hasil crack yg tersimpan untuk cek opsi %s]\n"%(U,til,O,U,O))
	for file in dirs:
